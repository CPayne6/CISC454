...

{
  ...
  char filename[ strlen(dir) + strlen(name) + 1 ];
  ...
  while(fscanf(file, "%s", buf) != EOF) {
	...
	char dir[ strlen(pathname) ];
	strcpy( dir, pathname );

	char *s = strrchr(dir, '/');
	if (s != NULL) s[1] = '\0'; else dir[0] = '\0';

	char filename[ strlen(dir) + strlen(buf) + 1 ];
...

{
  ...
  //
  // So we have to create *another* array of vertices and face
  // indices, where each vertex stores position, normal, and texture
  // coordinates and the face indices index into this new array.
  ...
  for (int i=0; i<groups.size(); i++)
    numTriangles += groups[i]->triangles.size();

  GLfloat vertexBuffer[ numTriangles * 3 * vertexSize ];
  GLuint faceIndexBuffer[ numTriangles * 3 ];

  unsigned int nVerts = 0;
  nFaces = 0;

  VertexSignature vertSig[ numTriangles * 3 ];

  for (int i=0; i<groups.size(); i++)
    for (int j=0; j<groups[i]->triangles.size(); j++) {
      wfTriangle *tri = groups[i]->triangles[j];
      for (int k=0; k<3; k++) {
  ...
  glVertexAttribPointer( attribIndex, 3, GL_FLOAT, GL_FALSE, vertexSize * sizeof(GLfloat), (const void *) accumulatedOffset );
  attribIndex++;
  accumulatedOffset += sizeof( vec3 );
  ...
  if (hasVertexTexCoords) {
    glEnableVertexAttribArray( attribIndex );
    glVertexAttribPointer( attribIndex, 2, GL_FLOAT, GL_FALSE, vertexSize * sizeof(GLfloat), (const void *) accumulatedOffset );
    attribIndex++;
    accumulatedOffset += sizeof( vec3 );
  }
...

{
  ...
  if (!VAOinitialized) {
    cerr << "wfModel::draw(): Must call setupVAO() before calling draw()" << endl;
    exit(1);
  }
...
void wfMaterial::setMaterial( bool useTextures, bool useMaterial )
...
{
  unsigned int power, exp, xdim, ydim;
  GLubyte *p, *q;
  ...
  GLubyte newTexMap[ xdim * ydim * (hasAlpha ? 4 : 3) ];
  ...
  gluBuild2DMipmaps( GL_TEXTURE_2D, (hasAlpha ? GL_RGBA : GL_RGB), xdim, ydim,
		     (hasAlpha ? GL_RGBA : GL_RGB), GL_UNSIGNED_BYTE, newTexMap ); // need the GLU library!
