...

{
  ...
  if (!out) 
    ...
    out << fovy << endl;
    out << orthoScale << endl;
    out << left << " " << bottom << " " << fNear << " " << fFar << endl;
    out << (projection == PERSPECTIVE ? 'p' : 'o') << endl;
...

{
  ...
  if (!in) 
    cerr << "Error: Failed to open the " << filename << " file, which should contain the viewing parameters.\n";
  else {
    char projType;
...

{
  ...
  message << '\0';
  glColor3f( 1, 0, .5 );
  xx = (int) (x - strlen(message.str())*8 - 16);
  yy = viewport[3] - 28 - y;
